...
interface AttendanceFormProps {
  preselectedEvent?: 'Friday Gathering' | 'Skills Acquisition' | 'Institute Cluster' | 'Missionary Preparatory Class' | 'Devotional' | 'Missionary Departure' | 'Movie Night';
}
...
const AttendanceForm: React.FC<AttendanceFormProps> = ({ preselectedEvent }) => {
  ...
  const [eventType, setEventType] = useState<'Friday Gathering' | 'Skills Acquisition' | 'Institute Cluster' | 'Missionary Preparatory Class' | 'Devotional' | 'Missionary Departure' | 'Movie Night'>(preselectedEvent || 'Friday Gathering');
  ...
  if (status.type === 'success') {
    return (
      <div className="max-w-md mx-auto animate-in fade-in zoom-in-95 duration-1000">
        <div className={`bg-white rounded-[2.5rem] p-8 text-center shadow-2xl border border-white relative overflow-hidden shadow-indigo-500/10`}>
          <>
          ...
          <div className="relative z-10">
            <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6 shadow-inner bg-amber-50`}>
    ...
    <div className="max-w-md mx-auto relative px-4 w-full">
      <div className="bg-white shadow-2xl shadow-slate-200/50 rounded-[2.5rem] overflow-hidden border border-white animate-in fade-in slide-in-from-bottom-12 duration-1000">
        <div className={`p-8 relative overflow-hidden `}>
          ...
          <header className="text-center mb-8 relative z-10">
            <div className={`w-12 h-12 mx-auto mb-4 rounded-2xl shadow-lg flex items-center justify-center bg-white p-1 shadow-indigo-500/20`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-5 relative z-10">
            ...
            <div className="w-full bg-slate-100 rounded-full h-1.5 mb-6 overflow-hidden">
              <div
                className={`h-full rounded-full transition-all duration-500 ease-out bg-gradient-to-r from-indigo-500 to-purple-500 shadow-[0_0_10px_rgba(99,102,241,0.5)]`}
              ...
              <div className="space-y-2 group animate-in fade-in slide-in-from-top-2 duration-300">
                ...
                <div className="relative flex items-center">
                  ...
                  <select
                    ...
                    className={`w-full bg-slate-50 border-0 ring-1 ring-slate-200 rounded-2xl pl-11 pr-4 py-4 focus:ring-2 focus:bg-white outline-none transition-all appearance-none font-bold text-slate-800 cursor-pointer text-sm shadow-sm hover:bg-slate-100 focus:ring-indigo-500`}
                  >
            ...
            <div className="space-y-2 group">
              ...
              <div className="relative flex items-center">
                ...
                <input
                  ...
                  className={`w-full bg-slate-50 border-0 ring-1 ring-slate-200 rounded-2xl pl-11 pr-4 py-4 focus:ring-2 focus:bg-white outline-none transition-all font-bold text-slate-800 text-sm shadow-sm hover:bg-slate-100 focus:ring-indigo-500`}
                />
              </div>
            </div>
            ...
            <div className="space-y-2 relative group" ref={suggestionRef}>
              ...
              <div className="relative flex items-center">
                ...
                <input
                  ...
                  className={`w-full bg-slate-50 border-0 ring-1 ring-slate-200 rounded-2xl pl-11 pr-4 py-4 focus:ring-2 focus:bg-white outline-none transition-all font-bold text-slate-800 placeholder:text-slate-400 text-sm shadow-sm hover:bg-slate-100 ${!ward ? 'opacity-50 cursor-not-allowed' : ''} focus:ring-indigo-500`}
                  disabled={!ward}
                />
              </div>
                  ...
                  <div className="p-3">
                    ...
                    {suggestions.map((s, idx) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 rounded-xl hover:bg-slate-50 transition-all flex items-center gap-3 group/btn hover:text-amber-600`}
                      >
            ...
            <button
              ...
              className={`w-full py-4 rounded-2xl font-black text-xs text-white transition-all transform shadow-lg relative group active:scale-[0.98] overflow-hidden mt-8 shadow-indigo-500/20 ${(isSubmitting || !ward || !name || !phone)
      ...
      <div className="text-center mt-6 opacity-60">
        <p className="text-[10px] text-slate-400 uppercase tracking-[0.3em] font-bold">
          {eventType === 'Friday Gathering' ? 'Young Single Adult Gathering' : (eventType === 'Skills Acquisition' ? 'YSA Skills Acquisition' : (eventType === 'Institute Cluster' ? 'Institute Cluster' : (eventType === 'Devotional' ? 'Stake Devotional' : (eventType === 'Missionary Departure' ? 'Missionary Departure' : (eventType === 'Movie Night' ? 'Movie Night' : 'Missionary Preparation')))))}
        </p>
      </div>
    </div>
  );
